...
function destinationReached(currentPosition, destinationPosition) {
  return (
    currentPosition[0] == destinationPosition[0] &&
    currentPosition[1] == destinationPosition[1]
  );
}
...
function moveBlizzard(blizzardMap) {
  const newBlizzardMap = blizzardMap.map((row) =>
  ...
  for (let row = 0; row < blizzardMap.length; row++) {
    for (let col = 0; col < blizzardMap[0].length; col++) {
      ...
      for (const blizzard of blizzardsAtCurrentPosition) {
        if (blizzard == "#") {
          newBlizzardMap[row][col] = ["#"];
        } else {
          const nextPosition = getNextBlizzardPosition(
            blizzardMap,
            row,
            col,
            blizzard
...
function getNextPositionsForHuman(blizzardMap, currentPosition) {
  ...
  for (const movement of humanMovements) {
...
function getMinutes(blizzardMap, startPosition, destinationPosition) {
  ...
  while (1) {
    steps++;
    const deDupePositions = blizzardMap.map((row) =>
    ...
    for (const currentPosition of currentPositions) {
      const nextPositions = getNextPositionsForHuman(
        blizzardMap,
        currentPosition
      );
...
function partOne(input) {
  const blizzardMap = input.map((row) =>
    row.split("").map((item) => (item == "." ? [] : [item]))
  );

  const startPosition = [0, 1];
  const destinationPosition = [
    blizzardMap.length - 1,
    blizzardMap[0].length - 1 - 1,
  ];

  const result = getMinutes(blizzardMap, startPosition, destinationPosition);
  console.log(result.steps);
}

function partTwo(input) {
  let blizzardMap = input.map((row) =>
    row.split("").map((item) => (item == "." ? [] : [item]))
  );
...
// function solve(gridInput) {
//   let blizzardMap = gridInput.map((row) =>
//     row.split("").map((item) => (item == "." ? [] : [item]))
//   );
//
//   const startPosition = [0, 1];
...
// partOne(sampleInput);
// getInput(24).then(partOne);
// partTwo(sampleInput);
getInput(24).then(partTwo);
