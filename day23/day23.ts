...
export function parseElves(input: string[]): Elves {
...
export function round(elves: Elves, startDir: Direction): Elves {
  ...
  // making moves
  for (const [claim, claimingElves] of Object.entries(claims)) {
...
// console.log(getEmptiness(parseElves(sampleInput)));
// getInput(23).then(parseElves).then(getEmptiness).then(console.log);

// console.log(simulateUntilChill(parseElves(sampleInput)));
getInput(23).then(parseElves).then(simulateUntilChill).then(console.log);
